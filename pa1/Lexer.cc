...
void Lexer::tokenize(){
    ...
    for(char c: binExp){
        if(Helper::isAlphaNum(c) and (wordStr.size() != 0 or (c != '0' and c != '1'))){
            ...
            switch(c){
                case ';':
                    this->tokenVector.push_back(std::make_pair(Token::semiColon, ";"));
                    break;
                case '+':
                ...
                case ')':
                    ...
                    break;
                case '0':
                    this->tokenVector.push_back(std::make_pair(Token::False, "0"));
                    break;
                case '1':
                    this->tokenVector.push_back(std::make_pair(Token::True, "1"));
                    break;
                case ',':
                    this->tokenVector.push_back(std::make_pair(Token::Comma, ","));
                    break;
                case ':':
                    this->tokenVector.push_back(std::make_pair(Token::Colon, ":"));
                    break;
                case ' ':
                    break;
                case '\n':
